...
Options include `--model`, `--base-url`, `--llm-provider auto|openrouter|agapi`,
and `--agent-profile chem|materials|general`.
...
Useful flags: `--profile`, `--llm-provider auto|openrouter|agapi`, `--model`,
and `--base-url`.
...
Useful flags: `--llm-provider auto|openrouter|agapi`, `--model`, `--base-url`,
`--html`, `--pretty/--no-pretty`, and `--quiet`.
...
`--llm-provider auto|openrouter|agapi`, `--model`, `--base-url`, `--html`,
